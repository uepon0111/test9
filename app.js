(() => {
  ...
  const LOCAL_EVENT_STORIES_URL = './data/eventStories.json';
  const REMOTE_EVENTS_URL = `${MASTER_BASE}/events.json`;
  const REMOTE_EVENT_STORIES_URL = `${MASTER_BASE}/eventStories.json`;
  ...
  async function getJsonWithFallback(localUrl, remoteUrl, label) {
    try {
      return await getJson(localUrl);
    } catch (localError) {
      try {
        return await getJson(remoteUrl);
      } catch (remoteError) {
        throw new Error(`${label} を取得できませんでした。\nローカル: ${localError.message}\nリモート: ${remoteError.message}`);
      }
    }
  }
  ...
  async function init() {
    setLoading(false);
    eventStatus.textContent = 'イベント一覧を読み込み中…';
    try {
      const [events, eventStories] = await Promise.all([
        getJsonWithFallback(LOCAL_EVENTS_URL, REMOTE_EVENTS_URL, 'events.json'),
        getJsonWithFallback(LOCAL_EVENT_STORIES_URL, REMOTE_EVENT_STORIES_URL, 'eventStories.json'),
      ]);

      if (!Array.isArray(events)) throw new Error('events.json の形式が配列ではありません。');
      if (!Array.isArray(eventStories)) throw new Error('eventStories.json の形式が配列ではありません。');

      state.events = sortEvents(events);
      state.eventStories = eventStories;
      renderEventList();
      footerStatus.textContent = `${state.events.length} イベント`;
      if (state.events.length === 0) {
        eventStatus.textContent = 'イベントが0件です。GitHub Actionsでdata/events.jsonを更新してください。';
      }
      applyQuery();
    } catch (err) {
      eventStatus.innerHTML = `<strong>イベント一覧を取得できませんでした。</strong><br><br>${esc(err.message).replaceAll('\n', '<br>')}<br><br>GitHub Pagesではリポジトリ内の <code>data/events.json</code> と <code>data/eventStories.json</code> を優先して使用します。\n      <br>この2ファイルがまだ無い場合は、Actionsの「Update master data」を手動実行してください。`;
