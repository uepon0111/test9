(() => {
  ...
  function buildSpeakerOptions(talks) {
    ...
    (talks || []).forEach(talk => {
      ...
      if (!name) return;
      const ids = (talk.characters || talk.TalkCharacters || []).map(c => typeof c === 'object' ? c.Character2dId : c).filter(v => v !== undefined);
  ...
  function renderDialogue() {
    ...
    dialogueListEl.innerHTML = filtered.map((talk, filteredIndex) => {
      const charIds = (talk.characters || []).map(c => typeof c === 'object' ? c.Character2dId : c).filter(v => v !== undefined);
      return `
